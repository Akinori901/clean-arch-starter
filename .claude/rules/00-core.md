...
**ルール・検証設定を触ったら、必ず違反を注入して落ちることを確認すること。**
確認せずに「入れた」と報告しない。

**設定があることと、効いていることは別である。**
実例として、フロントの境界検証は `import/resolver` が未設定だった期間、
`@/` 記法の違反を検知できていなかった（相対パスだけが別ルールで拾われ、
規約が推奨する書き方のほうが素通りしていた）。
設定ファイルは存在し、ルールも宣言されていたにもかかわらず、である。
